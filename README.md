...
- **`.github/workflows/build.yml`** - runs on every push/PR to `main` (and
  can be triggered manually from the Actions tab). Installs devkitARM,
  builds `platformer.gba`, and uploads it as a workflow artifact.
...
- **`.github/actions/setup-devkitarm`** - shared composite action that
  installs and caches the toolchain, used by both workflows so there's one
  place to update if the install steps ever need to change.
